...
pg_enc_text_out(PG_FUNCTION_ARGS)
{
	...
	char *pDst = (char *) palloc((pSrc->length + 1) * sizeof(char));
	int resp = ENCLAVE_IS_NOT_RUNNIG;
...
{	
	...
	// the actual size of dst can be different because of b64 convertion back and forth, but it will be less or equal
...
{	
	enc_str* enc_str1 = (enc_str*) PG_GETARG_POINTER(0);
	enc_str* enc_str2 = (enc_str*) PG_GETARG_POINTER(1);
	sgxErrorHandler(NOT_IMPLEMENTED_OPERATOR);
	...
	resp = enc_text_substring(enc_str1->src, enc_str1->length, enc_str2->src, enc_str2->length, pDst);
	sgxErrorHandler(resp);
	memcpy(&ans, pDst, INT32_LENGTH);
