...
int enc_int32_ops(int cmd, char * int1, char *int2, char *res) {
	...
	if (!FromBase64Fast((const BYTE*) int1, ENC_INT32_LENGTH_B64 - 1, int1_v.begin(), ENC_INT32_LENGTH))
				return BASE64DECODER_ERROR;

	if (!FromBase64Fast((const BYTE*) int2, ENC_INT32_LENGTH_B64 - 1, int2_v.begin(), ENC_INT32_LENGTH))
				return BASE64DECODER_ERROR;
	...
	while (true)
		{
			...
			{
				std::copy(&req->buffer[2*ENC_INT32_LENGTH], &req->buffer[3*ENC_INT32_LENGTH], int3_v.begin());
				resp = req->resp;
				if (!ToBase64Fast((const BYTE*) int3_v.begin(), ENC_INT32_LENGTH, res, ENC_INT32_LENGTH_B64))
	...
	while (true)
		{
			...
			{
				resp = req->resp;
				std::copy(&req->buffer[2*ENC_INT32_LENGTH], &req->buffer[2*ENC_INT32_LENGTH + INT32_LENGTH], &res[0]);
				spin_unlock(&req->is_done);
				break;
			}
		}
...
//		return resp;//IS_NOT_INITIALIZE;
	...
	while (true)
		{
			...
			{
				std::copy(&req->buffer[INT32_LENGTH], &req->buffer[INT32_LENGTH+ ENC_INT32_LENGTH], int1_v.begin());
				resp = req->resp;
				//spin_unlock(&req->is_done);
				break;
			}
	...
	if (!ToBase64Fast((const BYTE*) int1_v.begin(), ENC_INT32_LENGTH, pDst, ENC_INT32_LENGTH_B64))
						resp = BASE64DECODER_ERROR;
...
//		return resp;//IS_NOT_INITIALIZE;
	...
	std::array<BYTE, ENC_INT32_LENGTH> int1_v;
	if (!FromBase64Fast((const BYTE*) pSrc, ENC_INT32_LENGTH_B64 - 1, int1_v.begin(), ENC_INT32_LENGTH))
					return BASE64DECODER_ERROR;
	...
	while (true)
	{
		...
		{
				std::copy(&req->buffer[ENC_INT32_LENGTH], &req->buffer[ENC_INT32_LENGTH + INT32_LENGTH], &pDst[0]);
				resp = req->resp;
				spin_unlock(&req->is_done);
				break;
		}
