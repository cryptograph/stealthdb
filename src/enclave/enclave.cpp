...
int enclaveProcess (void* arg1) {
	...
	while (true) {
		...
		else
		{
			...
			{
				case CMD_INT64_PLUS:
					req->resp = enc_int32_add(req->buffer, ENC_INT32_LENGTH, req->buffer+ENC_INT32_LENGTH, ENC_INT32_LENGTH, req->buffer+ENC_INT32_LENGTH+ENC_INT32_LENGTH, ENC_INT32_LENGTH);
					break;

				case CMD_INT64_MINUS:
					req->resp = enc_int32_sub(req->buffer, ENC_INT32_LENGTH, req->buffer+ENC_INT32_LENGTH, ENC_INT32_LENGTH, req->buffer+ENC_INT32_LENGTH+ENC_INT32_LENGTH, ENC_INT32_LENGTH);
					break;

				case CMD_INT64_MULT:
					req->resp = enc_int32_mult(req->buffer, ENC_INT32_LENGTH, req->buffer+ENC_INT32_LENGTH, ENC_INT32_LENGTH, req->buffer+ENC_INT32_LENGTH+ENC_INT32_LENGTH, ENC_INT32_LENGTH);
					break;

				case CMD_INT64_DIV:
					req->resp = enc_int32_div(req->buffer, ENC_INT32_LENGTH, req->buffer+ENC_INT32_LENGTH, ENC_INT32_LENGTH, req->buffer+ENC_INT32_LENGTH+ENC_INT32_LENGTH, ENC_INT32_LENGTH);
					break;

				case CMD_INT64_EXP:
					req->resp = enc_int32_pow(req->buffer, ENC_INT32_LENGTH, req->buffer+ENC_INT32_LENGTH, ENC_INT32_LENGTH, req->buffer+ENC_INT32_LENGTH+ENC_INT32_LENGTH, ENC_INT32_LENGTH);
					break;

				case CMD_INT64_MOD:
					req->resp = enc_int32_mod(req->buffer, ENC_INT32_LENGTH, req->buffer+ENC_INT32_LENGTH, ENC_INT32_LENGTH, req->buffer+ENC_INT32_LENGTH+ENC_INT32_LENGTH, ENC_INT32_LENGTH);
					break;

				case CMD_INT64_CMP:
					req->resp = enc_int32_cmp(req->buffer, ENC_INT32_LENGTH, req->buffer+ENC_INT32_LENGTH, ENC_INT32_LENGTH, req->buffer+2*ENC_INT32_LENGTH, INT32_LENGTH);
					break;

				case CMD_INT64_ENC:
					req->resp = encrypt_bytes(req->buffer, INT32_LENGTH, req->buffer + INT32_LENGTH, ENC_INT32_LENGTH);
					break;

				case CMD_INT64_DEC:
					req->resp = decrypt_bytes(req->buffer, ENC_INT32_LENGTH, req->buffer + ENC_INT32_LENGTH, INT32_LENGTH);
					break;

				case CMD_FLOAT4_PLUS:
					req->resp = enc_float32_add(req->buffer, ENC_FLOAT4_LENGTH, req->buffer+ENC_FLOAT4_LENGTH, ENC_FLOAT4_LENGTH, req->buffer + 2*ENC_FLOAT4_LENGTH, ENC_FLOAT4_LENGTH);
					break;

				case CMD_FLOAT4_MINUS:
					req->resp = enc_float32_sub(req->buffer, ENC_FLOAT4_LENGTH, req->buffer+ENC_FLOAT4_LENGTH, ENC_FLOAT4_LENGTH, req->buffer + 2*ENC_FLOAT4_LENGTH, ENC_FLOAT4_LENGTH);
					break;

				case CMD_FLOAT4_MULT:
					req->resp = enc_float32_mult(req->buffer, ENC_FLOAT4_LENGTH, req->buffer+ENC_FLOAT4_LENGTH, ENC_FLOAT4_LENGTH, req->buffer + 2*ENC_FLOAT4_LENGTH, ENC_FLOAT4_LENGTH);
					break;

				case CMD_FLOAT4_DIV:
					req->resp = enc_float32_div(req->buffer, ENC_FLOAT4_LENGTH, req->buffer+ENC_FLOAT4_LENGTH, ENC_FLOAT4_LENGTH, req->buffer+ 2*ENC_FLOAT4_LENGTH, ENC_FLOAT4_LENGTH);
					break;

				case CMD_FLOAT4_EXP:
					req->resp = enc_float32_pow(req->buffer, ENC_FLOAT4_LENGTH, req->buffer+ENC_FLOAT4_LENGTH, ENC_FLOAT4_LENGTH, req->buffer+ 2*ENC_FLOAT4_LENGTH, ENC_FLOAT4_LENGTH);
					break;

				case CMD_FLOAT4_MOD:
					req->resp = enc_float32_mod(req->buffer, ENC_FLOAT4_LENGTH, req->buffer+ENC_FLOAT4_LENGTH, ENC_FLOAT4_LENGTH, req->buffer+ 2*ENC_FLOAT4_LENGTH, ENC_FLOAT4_LENGTH);
					break;

				case CMD_FLOAT4_CMP:
					req->resp = enc_float32_cmp(req->buffer, ENC_FLOAT4_LENGTH, req->buffer+ENC_FLOAT4_LENGTH, ENC_FLOAT4_LENGTH, req->buffer+2*ENC_FLOAT4_LENGTH, INT32_LENGTH);
					break;

				case CMD_FLOAT4_ENC:
					req->resp = encrypt_bytes(req->buffer, FLOAT4_LENGTH, req->buffer + FLOAT4_LENGTH, ENC_FLOAT4_LENGTH);
					break;

				case CMD_FLOAT4_DEC:
					req->resp = decrypt_bytes(req->buffer, ENC_FLOAT4_LENGTH, req->buffer + ENC_FLOAT4_LENGTH, FLOAT4_LENGTH);
					break;
				...
				case CMD_STRING_CMP:
					...
					req->resp = enc_text_cmp(src1, src_len, src2, src2_len, req->buffer+2*INT32_LENGTH + src_len + src2_len, INT32_LENGTH);
					break;
				...
				case CMD_STRING_ENC:
					...
					req->resp = encrypt_bytes(src1, src_len, req->buffer + src_len + 2*INT32_LENGTH, dst_len);
					//memcpy(req->buffer + INT32_LENGTH + src_len, &dst_len, INT32_LENGTH);
				...
				case CMD_STRING_LIKE:
					...
					memcpy(&src2_len, req->buffer+INT32_LENGTH + src_len,INT32_LENGTH);
					memcpy(src2, req->buffer+INT32_LENGTH + src_len +INT32_LENGTH, src2_len);
					req->resp = enc_text_substring(src1, src_len, src2, src2_len, req->buffer+2*INT32_LENGTH + src_len + src2_len, INT32_LENGTH);
					break;
				...
				case CMD_TIMESTAMP_CMP:
					req->resp = enc_timestamp_cmp(req->buffer, ENC_TIMESTAMP_LENGTH, req->buffer+ENC_TIMESTAMP_LENGTH, ENC_TIMESTAMP_LENGTH, req->buffer+2*ENC_TIMESTAMP_LENGTH, INT32_LENGTH);
					break;

				case CMD_TIMESTAMP_ENC:
					req->resp = encrypt_bytes(req->buffer, TIMESTAMP_LENGTH, req->buffer + TIMESTAMP_LENGTH, ENC_TIMESTAMP_LENGTH);
					break;

				case CMD_TIMESTAMP_DEC:
					req->resp = decrypt_bytes(req->buffer, ENC_TIMESTAMP_LENGTH, req->buffer + ENC_TIMESTAMP_LENGTH, TIMESTAMP_LENGTH);
					break;
			}
			//response->is_done = 1;
			req->is_done = 1;
